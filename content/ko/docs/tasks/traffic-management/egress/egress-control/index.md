...
weight: 10
aliases:
    - /ko/docs/tasks/egress.html
    - /ko/docs/tasks/egress
...
1. Configure [service entries](/ko/docs/reference/config/networking/service-entry/) to provide controlled access to external services.
...
*   Setup Istio by following the instructions in the [Installation guide](/ko/docs/setup/).
    Use the `demo` [configuration profile](/ko/docs/setup/additional-setup/config-profiles/) or otherwise
...
Istio has an [installation option](/ko/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig-OutboundTrafficPolicy-Mode),
...
[routing rules](/ko/docs/concepts/traffic-management/#routing-rules)
...
[intercepting](/ko/docs/concepts/traffic-management/)
external requests. To set up the bypass, change either the `global.proxy.includeIPRanges`
or the `global.proxy.excludeIPRanges` [configuration option](https://archive.istio.io/v1.4/ko/docs/reference/config/installation-options/) and
update the `istio-sidecar-injector` configuration map using the `kubectl apply` command. This can also
be configured on a pod by setting corresponding [annotations](/ko/docs/reference/config/annotations/) such as
...
Use the same command that you used to [install Istio](/ko/docs/setup/install/istioctl) and
...
[direct egress traffic through an egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/)
and review the security concerns described in the
[additional security considerations](/ko/docs/tasks/traffic-management/egress/egress-gateway/#additional-security-considerations)
section.
