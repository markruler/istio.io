...
aliases:
  - /ko/docs/examples/advanced-gateways/egress-gateway/
...
The [Control Egress Traffic](/ko/docs/tasks/traffic-management/egress/) task shows how to configure
...
Istio uses [ingress and egress gateways](/ko/docs/reference/config/networking/gateway/)
...
[destination rule lookup path](/ko/docs/ops/best-practices/traffic-management/#cross-namespace-configuration)
...
The [Kubernetes network policies](https://kubernetes.io/ko/docs/concepts/services-networking/network-policies/) can
...
[Kubernetes network policy](https://kubernetes.io/ko/docs/concepts/services-networking/network-policies/) to prevent
...
1.  Define a `NetworkPolicy` to limit the egress traffic from the `test-egress` namespace to traffic destined to
    ...
    {{< warning >}}
    [Network policies](https://kubernetes.io/ko/docs/concepts/services-networking/network-policies/)
...
1.  If [mutual TLS Authentication](/ko/docs/tasks/security/authentication/authn-policy/) is enabled, verify the correct certificate of the
    egress gateway:
