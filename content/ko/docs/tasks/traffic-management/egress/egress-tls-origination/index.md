...
test: yes
aliases:
  - /ko/docs/examples/advanced-gateways/egress-tls-origination/
---

The [Control Egress Traffic](/ko/docs/tasks/traffic-management/egress/) task demonstrates how external, i.e., outside of the
service mesh, HTTP and HTTPS services can be accessed from applications inside the mesh. As described in that task,
a [`ServiceEntry`](/ko/docs/reference/config/networking/service-entry/) is used to configure Istio
...
*   Start the [sleep]({{< github_tree >}}/samples/sleep) sample which will be used as a test source for external calls.
    ...
    If you have enabled [automatic sidecar injection](/ko/docs/setup/additional-setup/sidecar-injection/#automatic-sidecar-injection), deploy the `sleep` application:
...
using the same technique shown in the [Control Egress Traffic](/ko/docs/tasks/traffic-management/egress/) task.
