...
Kubernetes [ExternalName](https://kubernetes.io/ko/docs/concepts/services-networking/service/#externalname)
services and Kubernetes services with
[Endpoints](https://kubernetes.io/ko/docs/concepts/services-networking/service/#services-without-selectors)
...
[mutual TLS](/ko/docs/concepts/security/#mutual-tls-authentication). The external services are not part of an Istio service
...
following [Accessing Egress Services](/ko/docs/tasks/traffic-management/egress/egress-control/).
{{< /warning >}}
...
1.  Create a Kubernetes
    [ExternalName](https://kubernetes.io/ko/docs/concepts/services-networking/service/#externalname) service
...
1.  Access `httpbin.org` via the Kubernetes service's hostname from the source pod without Istio sidecar.
    Note that the _curl_ command below uses the [Kubernetes DNS format for services](https://v1-13.docs.kubernetes.io/ko/docs/concepts/services-networking/dns-pod-service/#a-records): `<service name>.<namespace>.svc.cluster.local`.
...
1.  In this example, unencrypted HTTP requests are sent to `httpbin.org`. For the sake of the example only, you disable
    ...
    to perform [Egress TLS origination](/ko/docs/tasks/traffic-management/egress/egress-tls-origination/) by Istio.
