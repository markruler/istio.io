...
- **Added** experimental [manifest and profile commands](/ko/docs/setup/install/istioctl/) to install and manage the Istio control plane for evaluation.

## Traffic management

- **Added** [automatic protocol determination](/ko/docs/ops/configuration/traffic-management/protocol-selection/) of HTTP or TCP for outbound traffic when ports are not named according to Istio’s [conventions](/ko/docs/ops/deployment/requirements/).
...
- **Improved** the Redis load balancer to now default to [`MAGLEV`](https://www.envoyproxy.io/ko/docs/envoy/v1.6.0/intro/arch_overview/load_balancing#maglev) when using the Redis proxy.
...
- **Improved** the [Sidecar API](/ko/docs/reference/config/networking/sidecar/#OutboundTrafficPolicy) to customize the `OutboundTrafficPolicy` policy.
...
- **Added** [labels]((/ko/docs/ops/configuration/mesh/secret-creation/) to control service account secret generation by namespace.
- **Added** SDS support to deliver the private key and certificates to each Istio control plane service.
- **Added** support for [introspection](/ko/docs/ops/diagnostic-tools/controlz/) to Citadel.
...
- **Improved** the ingress gateway to [load the trusted CA certificate from a separate secret](https://archive.istio.io/v1.3/ko/docs/tasks/traffic-management/ingress/secure-ingress-sds/#configure-a-mutual-tls-ingress-gateway) when using SDS.
...
- **Added** Access Log Service [ALS](https://www.envoyproxy.io/ko/docs/envoy/latest/api-v2/service/accesslog/v2/als.proto#grpc-access-log-service-als) support for Envoy gRPC.
- **Added** a Grafana dashboard for Citadel monitoring.
- **Added** [metrics](https://archive.istio.io/v1.3/ko/docs/reference/commands/sidecar-injector/#metrics) for monitoring the sidecar injector webhook.
...
- **Added** [`istioctl experimental manifest`](/ko/docs/reference/commands/istioctl/#istioctl-manifest) to manage the new experimental install manifests.
- **Added** [`istioctl experimental profile`](/ko/docs/reference/commands/istioctl/#istioctl-profile) to manage the new experimental install profiles.
- **Added** [`istioctl experimental metrics`](/ko/docs/reference/commands/istioctl/#istioctl-experimental-metrics)
- **Added** [`istioctl experimental describe pod`](/ko/docs/reference/commands/istioctl/#istioctl-experimental-describe-pod) to describe an Istio pod's configuration.
- **Added** [`istioctl experimental add-to-mesh`](/ko/docs/reference/commands/istioctl/#istioctl-experimental-add-to-mesh) to add Kubernetes services or virtual machines to an existing Istio service mesh.
- **Added** [`istioctl experimental remove-from-mesh`](/ko/docs/reference/commands/istioctl/#istioctl-experimental-remove-from-mesh) to remove Kubernetes services or virtual machines from an existing Istio service mesh.
- **Promoted** the [`istioctl experimental convert-ingress`](/ko/docs/reference/commands/istioctl/#istioctl-convert-ingress) command to `istioctl convert-ingress`.
- **Promoted** the [`istioctl experimental dashboard`](/ko/docs/reference/commands/istioctl/#istioctl-dashboard) command to `istioctl dashboard`.

## Miscellaneous

- **Added** new images based on [distroless](/ko/docs/ops/configuration/security/harden-docker-images/) base images.
