...
- **Added** [deny semantics](/ko/docs/tasks/security/authorization/authz-deny) and [exclusion matching](/ko/docs/concepts/security/#exclusion-matching) to Authorization Policy.
- **Graduated** [auto mutual TLS](/ko/docs/tasks/security/authentication/authn-policy/#auto-mutual-tls) from alpha to beta. This feature is now enabled by default.
- **Improved** [SDS security](https://www.envoyproxy.io/ko/docs/envoy/latest/configuration/security/secret) by merging Node Agent with Pilot Agent as Istio Agent and removing cross-pod UDS, which no longer requires users to deploy Kubernetes pod security policies for UDS connections.
- **Improved** Istio by including certificate provisioning functionality within Istiod.
- **Added** Support Kubernetes [`first-party-jwt`](https://kubernetes.io/ko/docs/reference/access-authn-authz/authentication/#service-account-tokens) as a fallback token for CSR authentication in clusters where [`third-party-jwt`](https://kubernetes.io/ko/docs/tasks/configure-pod-container/configure-service-account/#service-account-token-volume-projection) is not supported.
...
- **Replaced** the alpha `IstioControlPlane` API with the new [`IstioOperator`](/ko/docs/reference/config/istio.operator.v1alpha1/) API to align with existing `MeshConfig` API.
...
- **Graduated** [`Istioctl Analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) out of experimental.
...
- **Added** a new flag `-A|--all-namespaces` to [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) to analyze the entire cluster.
- **Added** support for analyzing content passed via `stdin` to [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/).
- **Added** [`istioctl analyze -L`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) to show a list of all analyzers available.
- **Added** the ability to suppress messages from [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/).
- **Added** structured format options to [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/).
- **Added** links to relevant documentation to [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) output.
- **Updated** annotation methods provided by Istio API in [`Istioctl Analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/).
- **Updated** [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) now loads files from a directory.
- **Updated** [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) to try to associate message with their source filename.
- **Updated** [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) to print the namespace that is being analyzed.
- **Updated** [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) to analyze in-cluster resources by default.
- **Fixed** bug where [`istioctl analyze`](/ko/docs/ops/diagnostic-tools/istioctl-analyze/) suppressed cluster-level resource messages.
- **Added** support for multiple input files to `istioctl manifest`.
- **Replaced** the `IstioControlPlane` API with the `IstioOperator` API.
- **Added** selector for [`istioctl dashboard`](/ko/docs/reference/commands/istioctl/#istioctl-dashboard).
- **Added** support for slices and lists in [`istioctl manifest --set`](/ko/docs/reference/commands/istioctl/#istioctl-manifest) flag.
- **Added** support for [`istioctl manifest`](/ko/docs/reference/commands/istioctl/#istioctl-manifest) to read profiles from `stdin`.
