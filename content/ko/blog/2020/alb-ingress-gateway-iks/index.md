...
In this blog post I show how to configure the [Ingress Application Load Balancer (ALB)](https://cloud.ibm.com/ko/docs/containers?topic=containers-ingress-about)
...
[Ingress](https://kubernetes.io/ko/docs/concepts/services-networking/ingress/) resource with
[ALB-specific annotations](https://cloud.ibm.com/ko/docs/containers?topic=containers-ingress_annotation). IKS provides a
DNS domain name, a TLS certificate that matches the domain, and a private key for the certificate. IKS stores the
certificates and the private key in a [Kubernetes secret](https://kubernetes.io/ko/docs/concepts/configuration/secret/).

When you start using Istio in your IKS cluster, the recommended method to send traffic to your Istio enabled workloads
is by using the [Istio Ingress Gateway](/ko/docs/tasks/traffic-management/ingress/ingress-control/) instead of using the
[Kubernetes Ingress](https://kubernetes.io/ko/docs/concepts/services-networking/ingress/). One of the main reasons to use
...
[register a new DNS subdomain](https://cloud.ibm.com/ko/docs/containers?topic=containers-loadbalancer_hostname) for the
Istio gateway's IP with an IKS command. The domain is in the following
[format](https://cloud.ibm.com/ko/docs/containers?topic=containers-loadbalancer_hostname#loadbalancer_hostname_format):
