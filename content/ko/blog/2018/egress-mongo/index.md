...
You use the [Istio Bookinfo sample application](/ko/docs/examples/bookinfo/), the version in which the book
...
modify the [Bookinfo sample application](/ko/docs/examples/bookinfo/) to use your database.
...
To demonstrate the scenario of using an external database, you start with a Kubernetes cluster with [Istio installed](/ko/docs/setup/getting-started/). Then you deploy the
[Istio Bookinfo sample application](/ko/docs/examples/bookinfo/), [apply the default destination rules](/ko/docs/examples/bookinfo/#apply-default-destination-rules), and
...
service to _ratings v2_ that uses your database.
    ...
    [virtual services](/ko/docs/reference/config/networking/virtual-service/). These virtual services are
    specified in `samples/bookinfo/networking/virtual-service-ratings-mongodb.yaml` of an Istio release archive.
    ***Important:*** make sure you
    [applied the default destination rules](/ko/docs/examples/bookinfo/#apply-default-destination-rules) before running the
     following command.
...
[directly](/ko/docs/tasks/traffic-management/egress/egress-control/#direct-access-to-external-services), bypassing the Istio sidecar
proxies.
...
[egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/#use-case), for example if you do not have a
...
1.  Define a TCP mesh-external service entry:
    ...
    [MongoDB related statistics](https://www.envoyproxy.io/ko/docs/envoy/latest/configuration/listeners/network_filters/mongo_proxy_filter#statistics).
...
[egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/#use-case). The sidecar proxy routes TCP
...
1.  [Deploy Istio egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/#deploy-istio-egress-gateway).
...
In case you [do not need an egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/#use-case), follow the
...
In this section you handle the case when you need to direct the traffic through an
[egress gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/#use-case). The sidecar proxy routes TLS
...
1.  Create an egress `Gateway` for your MongoDB service, and destination rules and virtual services
    ...
    If you want to enable [mutual TLS Authentication](/ko/docs/tasks/security/authentication/authn-policy/) between the sidecar proxies of
...
gateway with
[an additional SNI proxy](/ko/docs/tasks/traffic-management/egress/wildcard-egress-hosts/#wildcard-configuration-for-arbitrary-domains).
