...
A simple way to see this type of approach in action is to first setup your Kubernetes environment using the [Platform Setup](/ko/docs/setup/platform-setup/) instructions, and then install the **minimal** Istio profile using [Helm](https://archive.istio.io/1.4/ko/docs/setup/install/helm/), including only the traffic management components (ingress gateway, egress gateway, Pilot). The following example uses [Google Kubernetes Engine](https://cloud.google.com/gke).

First, setup and configure [GKE](/ko/docs/setup/platform-setup/gke/):
...
Next, [install Helm](https://helm.sh/ko/docs/intro/install/) and [generate a minimal Istio install](https://archive.istio.io/1.4/ko/docs/setup/install/helm/) -- only traffic management components:
