...
In this blog post, I modify the [Istio Bookinfo Sample Application](/ko/docs/examples/bookinfo/) to fetch book details from
an external web service ([Google Books APIs](https://developers.google.com/books/ko/docs/v1/getting_started)). I show how
...
To demonstrate the scenario of consuming an external web service, I start with a Kubernetes cluster with [Istio installed](/ko/docs/setup/getting-started/). Then I deploy
[Istio Bookinfo Sample Application](/ko/docs/examples/bookinfo/). This application uses the _details_ microservice to fetch
...
[mutual TLS](/ko/docs/concepts/security/#mutual-tls-authentication) enabled. The Bookinfo configuration files reside in the
...
[Bookinfo sample application](/ko/docs/examples/bookinfo/).
...
[Deploying the application](/ko/docs/examples/bookinfo/#deploying-the-application),
[Confirm the app is running](/ko/docs/examples/bookinfo/#confirm-the-app-is-accessible-from-outside-the-cluster),
[Apply default destination rules](/ko/docs/examples/bookinfo/#apply-default-destination-rules)
sections, and
[change Istio to the blocking-egress-by-default policy](/ko/docs/tasks/traffic-management/egress/egress-control/#change-to-the-blocking-by-default-policy).

## Bookinfo with HTTPS access to a Google Books web service

Deploy a new version of the _details_ microservice, _v2_, that fetches the book details from [Google Books APIs](https://developers.google.com/books/ko/docs/v1/getting_started). Run the following command; it sets the
...
Note that the virtual service relies on a destination rule that you created in the [Apply default destination rules](/ko/docs/examples/bookinfo/#apply-default-destination-rules) section.

Access the web page of the application, after
[determining the ingress IP and port](/ko/docs/examples/bookinfo/#determine-the-ingress-ip-and-port).
...
[mesh-external service entry](/ko/docs/reference/config/networking/service-entry/).
...
([Books](https://developers.google.com/books/ko/docs/v1/getting_started),
...
[Books APIs](https://developers.google.com/books/ko/docs/v1/getting_started) is allowed. Suppose you want to monitor the
...
In that URL, [Books APIs](https://developers.google.com/books/ko/docs/v1/getting_started) is specified by the path segment
...
inside or outside an Istio service mesh. This contradicts the Istio design goal of [maximizing transparency](/ko/docs/ops/deployment/architecture/#design-goals). Sometimes you need to compromise...
...
1.  Deploy a version of _details v2_ that sends an HTTP request to
    [Google Books APIs](https://developers.google.com/books/ko/docs/v1/getting_started). The `DO_NOT_ENCRYPT` variable
    is set to true in
...
1.  [Enable Envoy’s access logging](/ko/docs/tasks/observability/logs/access-log/#enable-envoy-s-access-logging)
...
[the mutual TLS](/ko/docs/concepts/security/#mutual-tls-authentication) applied by Istio. The TLS origination for the
