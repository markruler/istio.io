...
into microservices deployed in an Istio service mesh. The applications access pages of various topics from _cnn.com_: [edition.cnn.com/politics](https://edition.cnn.com/politics), [edition.cnn.com/sport](https://edition.cnn.com/sport) and  [edition.cnn.com/health](https://edition.cnn.com/health). The organization [configures Istio to allow access to edition.cnn.com](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination/) and everything works fine. However, at some
...
* The [Control Egress Traffic](/ko/docs/tasks/traffic-management/egress/) task demonstrates how external (outside the
  Kubernetes cluster) HTTP and HTTPS services can be accessed by applications inside the mesh.
* The [Configure an Egress Gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway/) example describes how to configure
  Istio to direct egress traffic through a dedicated gateway service called _egress gateway_.
* The [Egress Gateway with TLS Origination](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination/) example
...
* The [Visualizing Metrics with Grafana](/ko/docs/tasks/observability/metrics/using-istio-dashboard/)
  describes the Istio Dashboard to monitor mesh traffic.
* The [Basic Access Control](https://istio.io/v1.6/ko/docs/tasks/policy-enforcement/denial-and-list/) task shows how to control access to
  in-mesh services.
* The [Denials and White/Black Listing](https://istio.io/v1.6/ko/docs/tasks/policy-enforcement/denial-and-list/) task shows how to configure
...
Follow the steps in the [Egress Gateway with TLS Origination](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination/) example, **with mutual TLS authentication enabled**, without
the [Cleanup](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination//#cleanup) step.
...
_egress gateway_, as described in the [Egress Gateway with TLS Origination](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination/)
...
[stdio](https://istio.io/v1.6/ko/docs/reference/config/policy-and-telemetry/adapters/stdio/) `handlers`, one for logging forbidden access
...
1.  Redefine your `VirtualService` for _edition.cnn.com_:
    ...
    (/ko/docs/tasks/traffic-management/egress/egress-gateway/#additional-security-considerations)). You don't want any tampering
    with your policies.
...
for example
[white lists or black lists](https://istio.io/v1.6/ko/docs/tasks/policy-enforcement/denial-and-list/#attribute-based-whitelists-or-blacklists)
of allowed/forbidden URL paths, respectively.
[Policy Rules](https://istio.io/v1.6/ko/docs/reference/config/policy-and-telemetry/istio.policy.v1beta1/) allow specifying complex conditions,
specified in a [rich expression language](https://istio.io/v1.6/ko/docs/reference/config/policy-and-telemetry/expression-language/), which
includes AND and OR logical operators. The rules can be reused for both logging and policy checks. More advanced users
may want to apply [Istio Role-Based Access Control](/ko/docs/concepts/security/#authorization).
...
1.  Replace the `VirtualService` for _edition.cnn.com_ with your previous version from the [Configure an Egress Gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway-tls-origination/#perform-tls-origination-with-an-egress-gateway) example:
...
[`Listchecker` adapter](https://istio.io/v1.6/ko/docs/reference/config/policy-and-telemetry/adapters/list/), its whitelist
...
[Service Account](https://kubernetes.io/ko/docs/tasks/configure-pod-container/configure-service-account/) to access any topic of _cnn.com_, without being monitored. You'll see how this requirement can be configured in Istio.
...
and the egress gateway, the [identity of the source](/ko/docs/concepts/security/#istio-identity) is also known.
...
1.  Perform the instructions in [Cleanup](/ko/docs/tasks/traffic-management/egress/egress-gateway//#cleanup) section of the
[Configure an Egress Gateway](/ko/docs/tasks/traffic-management/egress/egress-gateway//) example.
